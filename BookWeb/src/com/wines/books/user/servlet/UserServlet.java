package com.wines.books.user.servlet;
...
import com.wines.books.user.service.UserService;
...
public class UserServlet extends BaseServlet {
	...
	public String regist(HttpServletRequest req, HttpServletResponse resp)
		...
		/*
		 * 2、校验之，如果校验结果为失败，保存错误信息，返回到regist.jsp显示
		 */
	 ...
	 * @throws IOException
	 */
	public String ajaxValidatePassword(HttpServletRequest req, HttpServletResponse resp)
			throws ServletException, IOException {
		String loginpass = req.getParameter("loginpass");
		boolean b = userService.ajaxValidatePassword(loginpass);
		resp.getWriter().print(b);
		return null;
	}
	...
	public String updatePassword(HttpServletRequest req, HttpServletResponse resp)
		 ...
		 * 1、封装表单数据到user中
		 * 2、从session中获取uid
		 * 3、使用uid和表单中的oldPass、newPass来调用service方法
		 *  >如果出现异常，保存异常信息到request中，转发到pwd.jsp
		 * 4、保存成功信息到request中，转发到msg.jsp
		 */
		User formUser = CommonUtils.toBean(req.getParameterMap(), User.class);
	...
	public String activation(HttpServletRequest req, HttpServletResponse resp)
		...
		try {
			...
			req.setAttribute("code", "success");
			req.setAttribute("msg", "恭喜您，激活成功，可以马上进行登录");
		} catch (UserException e) {
			//说明service抛出了异常
			req.setAttribute("msg", e.getMessage());
